    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                weekly_bonus_claimed TEXT DEFAULT '',
        ...
        # Migrate existing users table if needed (check each column individually)
        columns_to_add = [
            ("total_wagered", "INTEGER DEFAULT 0"),
            ("total_won", "INTEGER DEFAULT 0"),
            ("games_played", "INTEGER DEFAULT 0"),
        ...
        for column_name, column_definition in columns_to_add:
            try:
                cur = await db.execute(f"SELECT {column_name} FROM users LIMIT 1")
                await cur.fetchone()
            except:
                # Column doesn't exist, add it
                await db.execute(f"ALTER TABLE users ADD COLUMN {column_name} {column_definition}")
                logger.info(f"Added column {column_name} to users table")
        
        # Update existing users with current timestamps if they're empty
        try:
            current_time = datetime.now().isoformat()
            await db.execute("UPDATE users SET created_at = ?, last_active = ? WHERE created_at = '' OR created_at IS NULL", 
...
async def create_user(user_id: int, username: str, referrer_id: int = None):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, username, 100, current_time, current_time, referrer_id))
        ...
        await db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (transaction_id, user_id, "registration_bonus", 100, "Welcome bonus for new user", current_time))
        ...
        if referrer_id:
            ...
            if referrer:
                ...
                await db.execute("""
                    INSERT INTO transactions (id, user_id, transaction_type, amount, description, timestamp) 
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (ref_transaction_id, referrer_id, "referral_bonus", REFERRAL_BONUS, f"Referral bonus for inviting user {user_id}", current_time))
...
async def add_balance(user_id: int, amount: int, transaction_type: str = "game_win", description: str = ""):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            INSERT INTO transactions (id, user_id, transaction_type, amount, description, timestamp) 
            VALUES (?, ?, ?, ?, ?, ?)
        """, (transaction_id, user_id, transaction_type, amount, description, current_time))
...
async def get_user_statistics(user_id: int) -> dict:
    ...
    return {
        "user": user,
        "game_stats": [{"game_type": row[0], "games": row[1], "total_bet": row[2], "total_win": row[3], "avg_multiplier": row[4]} for row in game_stats],
        "recent_transactions": [{"transaction_type": row[0], "amount": row[1], "description": row[2], "timestamp": row[3]} for row in recent_transactions],
        "achievements": [{"achievement_name": row[0], "earned_at": row[1]} for row in achievements],
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not existing:
        ...
        user = await get_user(user_id)  # Get fresh user data
        welcome_message = f"🎉 *Welcome bonus: 100 chips awarded!*"
    else:
        user = existing
        welcome_message = f"👋 *Welcome back, {user.get('username', 'Player')}!*"
    ...
    if user['balance'] >= VIP_DIAMOND_REQUIRED:
        vip_status = "💎 Diamond VIP"
    elif user['balance'] >= VIP_GOLD_REQUIRED:
        vip_status = "🥇 Gold VIP"
    elif user['balance'] >= VIP_SILVER_REQUIRED:
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("🎮 Mini App Centre", callback_data="mini_casino_app"), InlineKeyboardButton("💰 Check Balance", callback_data="show_balance")],
        [InlineKeyboardButton("🎁 Weekly Bonus", callback_data="weekly_bonus"), InlineKeyboardButton("📊 My Statistics", callback_data="show_stats")],
        [InlineKeyboardButton("🏆 Leaderboard", callback_data="show_leaderboard"), InlineKeyboardButton("⚙️ Settings", callback_data="settings_menu")],
...
async def weekly_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Trigger weekly bonus callback logic
    class FakeQuery:
    ...
    # Create a fake update with callback query
    fake_update = type('Update', (), {
        'callback_query': FakeQuery(user_id),
        'message': update.message
    })()
    
    await weekly_bonus_callback(fake_update, context)
...
async def weekly_bonus_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle weekly bonus callback from inline keyboard"""
    query = update.callback_query
    user_id = query.from_user.id
    ...
    # Check if weekly bonus already claimed this week
    now = datetime.utcnow()
    week_start = now - timedelta(days=now.weekday())  # Monday of current week
    week_start_str = week_start.strftime('%Y-%m-%d')
    
    if user.get('weekly_bonus_claimed', '') >= week_start_str:
        await query.answer("🚫 Weekly bonus already claimed this week!", show_alert=True)
        return
    
    # Calculate bonus based on last week's bets
    ...
    # Calculate 5% bonus
    bonus_amount = int(total_weekly_bets * WEEKLY_BONUS_RATE)
...
📊 *Details:*
• You didn't place any bets last week
• Bonus = 5% of all bets from previous week
• Start playing games to earn next week's bonus!

💡 *Tip:* Play more games this week to get a bigger bonus next Monday!
"""
    else:
        # Update user's weekly bonus claimed date and add balance
        await add_balance(user_id, bonus_amount)
        
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("""
                UPDATE users 
                SET weekly_bonus_claimed = ? 
                WHERE id = ?
            """, (now.strftime('%Y-%m-%d'), user_id))
            await db.commit()
        
        text = f"""
🎁 *Weekly Balance Bonus Claimed!* 🎁

💰 *You received:* {bonus_amount:,} chips!

📊 *Bonus Details:*
• Total bets last week: {total_weekly_bets:,} chips
• Bonus rate: {WEEKLY_BONUS_RATE*100}% of weekly bets
• Next bonus: Available next Monday

🎯 *Your Updated Balance:* {user['balance'] + bonus_amount:,} chips

Keep playing to earn an even bigger bonus next week!
"""
...
async def stake_originals_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Stake's Original Games category - Main Hub"""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    user = await get_user(user_id)
    ...
    total_games_played = user.get('games_played', 0)
    total_wagered = user.get('total_wagered', 0)
    profit_loss = user.get('total_won', 0) - total_wagered
    
    # Get user's favorite game (mock data for now)
    favorite_game = "🚀 CRASH" if total_games_played > 0 else "None yet"
    
    text = f"""
🔥 **STAKE ORIGINALS** 🔥
*Premium In-House Games*

� **Player Profile:**
• Balance: **{user['balance']:,}** chips
• Games Played: **{total_games_played}**
• Total Wagered: **{total_wagered:,}** chips  
• Profit/Loss: **{profit_loss:+,}** chips
• Favorite Game: {favorite_game}

🎮 **Featured Games:**

**🚀 CRASH** 
*Watch multiplier climb • Cash out before crash*
• RTP: **99%** • Max Win: **10,000x**
• Players Online: **2,847**

**💣 MINES**
*Navigate minefield • Massive multipliers*
• RTP: **97%** • Max Win: **24x** per mine
• Difficulty: **HIGH** 

**🏀 PLINKO**
*Drop balls through pegs • Pure luck*
• RTP: **97.5%** • Max Win: **1,000x**
• Paylines: **16 buckets**

**🃏 HI-LO**
*Predict higher/lower cards • Skill based*
• RTP: **98%** • Max Win: **100x**
• Strategy: **Card counting**

**🎲 LIMBO**
*Under/Over target number*
• RTP: **99%** • Max Win: **1,000,000x**
• Target: **Customizable**

**🎡 WHEEL**
*Spin fortune wheel • Instant wins*
• RTP: **96%** • Max Win: **50x**
• Segments: **54 options**
...
🏆 **Today's Big Wins:**
• Anonymous won **85,000** on CRASH (180x)
• Player_247 won **12,500** on MINES (25x)
• VIP_User won **7,800** on PLINKO (390x)

Choose your game:
"""
    
    keyboard = [
        [InlineKeyboardButton("🚀 CRASH", callback_data="play_crash"), InlineKeyboardButton("💣 MINES", callback_data="play_mines")],
        [InlineKeyboardButton("🏀 PLINKO", callback_data="play_plinko"), InlineKeyboardButton("🃏 HI-LO", callback_data="play_hilo")],
        [InlineKeyboardButton("🎲 LIMBO", callback_data="play_limbo"), InlineKeyboardButton("🎡 WHEEL", callback_data="play_wheel")],
        [InlineKeyboardButton("🎰 AUTO PLAY", callback_data="auto_play_menu"), InlineKeyboardButton("📊 STATISTICS", callback_data="game_statistics")],
        [InlineKeyboardButton("🎮 RANDOM GAME", callback_data="random_originals"), InlineKeyboardButton("🏆 LEADERBOARD", callback_data="originals_leaderboard")],
...
📊 **Account Summary:**
• Total Wagered: **{total_wagered:,}** chips
• Total Won: **{total_won:,}** chips
• Net P/L: **{profit_loss:+,}** chips
...
"""
    ...
    if transactions:
        for trans_type, amount, desc, timestamp in transactions[:5]:
            emoji = "🎁" if "bonus" in trans_type else "🎮" if "game" in trans_type else "💰"
            amount_str = f"+{amount:,}" if amount > 0 else f"{amount:,}"
            text += f"• {emoji} {amount_str} - {trans_type.replace('_', ' ').title()}\n"
    else:
...
🎁 **Available Bonuses:**
• Weekly Bonus: {"✅ Claimed" if user.get('weekly_bonus_claimed') else "🎁 Available"}
• VIP Rewards: {"👑 Available" if balance >= 1000 else "❌ Reach 1,000 chips"}
• Referral Bonus: 🔗 Invite friends

🔒 **Security:**
• Account Status: ✅ Verified
• Security Level: {user.get('security_level', 'low').title()}
• Last Active: Just now
"""
    
    keyboard = [
        [InlineKeyboardButton("🎁 Claim Weekly Bonus", callback_data="weekly_bonus"), InlineKeyboardButton("📊 Full Transaction History", callback_data="transaction_history")],
        [InlineKeyboardButton("🔗 Referral Program", callback_data="referral_program"), InlineKeyboardButton("⚙️ Account Settings", callback_data="account_settings")],
        [InlineKeyboardButton("🎮 Play Games", callback_data="mini_casino_app"), InlineKeyboardButton("🔙 Back to Casino", callback_data="mini_casino_app")]
    ]
...
async def mini_app_centre_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show the Mini App Centre interface"""
    user = update.effective_user
    if not user:
        await update.message.reply_text("❌ Could not identify user. Please try again.")
        return
...
async def show_mini_app_centre(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show the comprehensive Mini App Centre"""
    user_id = update.effective_user.id
    user = await get_user(user_id)
    ...
    if balance >= VIP_DIAMOND_REQUIRED:
        level_info = "💎 DIAMOND VIP"
        level_emoji = "💎"
    elif balance >= VIP_GOLD_REQUIRED:
        level_info = "🥇 GOLD VIP"
        level_emoji = "🥇"
    elif balance >= VIP_SILVER_REQUIRED:
        level_info = "🥈 SILVER VIP"
        level_emoji = "🥈"
    else:
        level_info = "🎰 REGULAR PLAYER"
        level_emoji = "🎲"
...
{level_emoji} **{first_name}** | {level_info}
💰 **Balance:** {balance:,} chips
🎯 **Level Progress:** {min(balance, VIP_SILVER_REQUIRED)}/{VIP_SILVER_REQUIRED} to Silver
