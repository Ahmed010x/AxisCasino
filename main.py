...
async def update_daily_house_stats(bet_amount: float, win_amount: float) -> bool:
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            cursor = await db.execute("PRAGMA table_info(house_balance)")
            columns = [row[1] for row in await cursor.fetchall()]
            
            if 'games_played_today' in columns and 'revenue_today' in columns and 'profit_today' in columns:
                await db.execute("""
                    UPDATE house_balance 
                ...
                # Columns don't exist yet, skip daily stats update
                logger.debug("Daily house stats columns not available, skipping update")
                return True
...
# --- Start Command and Main Callback Handler ---
...
"""
    ...
    # Add admin options for admins/owners
    if is_admin(user_id) or is_owner(user_id):
        keyboard.append([InlineKeyboardButton("🔧 Admin Panel", callback_data="admin_panel")])
    
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)

async def handle_main_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
💵 <b>Total Wagered:</b> {await format_usd(user.get('total_wagered', 0.0))}
🏆 <b>Total Won:</b> {await format_usd(user.get('total_won', 0.0))}
...
"""
        ...
        user = await get_user(user_id)
        if user:
            win_rate = (user.get('total_won', 0.0) / user.get('total_wagered', 1.0)) * 100 if user.get('total_wagered', 0) > 0 else 0
...
🔥 <b>Win Streak:</b> {user.get('win_streak', 0)}
📈 <b>Max Win Streak:</b> {user.get('max_win_streak', 0)}
💎 <b>Biggest Win:</b> {await format_usd(user.get('biggest_win', 0.0))}
...
"""
        ...
        text = """
ℹ️ <b>Casino Help</b> ℹ️

🎰 <b>Slots:</b> Match 3 symbols to win
🎲 <b>Dice:</b> Bet on High (8-12) or Low (2-7)
🃏 <b>Blackjack:</b> Get as close to 21 as possible
🎡 <b>Roulette:</b> Bet on colors, numbers, or even/odd
...
🔧 <b>Support:</b> Contact {SUPPORT_CHANNEL}
"""
    ...
    elif query.data == "admin_panel" and (is_admin(user_id) or is_owner(user_id)):
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get basic stats
    house_data = await get_house_balance()
    balance_str = await format_usd(house_data.get('balance', 0.0))
    
    text = f"""
🔧 <b>ADMIN PANEL</b> 🔧

🏦 <b>House Balance:</b> {balance_str}
🎮 <b>Games Today:</b> {house_data.get('games_played_today', 0)}
💰 <b>Revenue Today:</b> {await format_usd(house_data.get('revenue_today', 0.0))}
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("🏦 House Balance", callback_data="admin_house_balance")],
...
# --- Game Integration with House Balance ---
...
async def start_game_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user:
        user = await create_user(user_id, update.effective_user.username or "User")
...
async def play_simple_slots(user_id: int, bet_amount: float) -> str:
    """Play a simple slots game"""
    symbols = ['🍒', '🍋', '🍊', '🔔', '💎']
    ...
    for i, symbol in enumerate(symbols):
        weighted_symbols.extend([symbol] * weights[i])
    
    # Generate reels
    reel1 = random.choice(weighted_symbols)
    reel2 = random.choice(weighted_symbols)
    reel3 = random.choice(weighted_symbols)
    ...
    if reel1 == reel2 == reel3:
        # Three matching
        idx = symbols.index(reel1)
        multiplier = payouts[idx]
        win_amount = bet_amount * multiplier
        result_text = f"🎉 JACKPOT! {reel1}{reel2}{reel3} - {multiplier}x!"
    elif reel1 == reel2 or reel2 == reel3 or reel1 == reel3:
        # Two matching
        win_amount = bet_amount * 0.5
        result_text = f"✨ Two match! {reel1}{reel2}{reel3} - 0.5x!"
    else:
        # No match
        win_amount = 0
        result_text = f"💔 No match: {reel1}{reel2}{reel3}"
    ...
    await update_daily_house_stats(bet_amount, win_amount)
...
📊 <b>Net:</b> ${net_result:+.2f}
💳 <b>Balance:</b> {balance_str}
"""

async def play_simple_dice(user_id: int, bet_amount: float, bet_type: str) -> str:
    ...
    if bet_type == 'high':
        is_win = total >= 8
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "HIGH (8-12)"
    elif bet_type == 'low':
        is_win = total <= 7
        win_amount = bet_amount * 2.0 if is_win else 0
        ...
        is_win = total == 7
        win_amount = bet_amount * 6.0 if is_win else 0
        bet_desc = "LUCKY 7"
    ...
    # Log game
    result_text = f"{'WIN' if is_win else 'LOSE'} - {bet_desc}"
...
{dice_emojis[die1]} {dice_emojis[die2]} = <b>{total}</b>

🎯 <b>Your Bet:</b> {bet_desc}
...
async def play_simple_blackjack(user_id: int, bet_amount: float) -> str:
    ...
    def card_value(rank):
        if rank in ['J', 'Q', 'K']:
            ...
            return 11  # We'll handle aces properly
        else:
            return int(rank)
    ...
    # Simple dealer logic (dealer hits until 17+)
    ...
    if player_blackjack and dealer_blackjack:
        result_desc = "PUSH - Both Blackjack!"
        win_amount = bet_amount  # Return bet
    elif player_blackjack:
        result_desc = "BLACKJACK!"
        win_amount = bet_amount * 2.5  # 3:2 payout
        is_win = True
    elif dealer_blackjack:
        result_desc = "Dealer Blackjack"
        win_amount = 0
    elif player_value > 21:
        result_desc = "BUST"
        win_amount = 0
    elif dealer_value > 21:
        result_desc = "Dealer Bust - YOU WIN!"
        ...
        result_desc = "Dealer Wins"
        win_amount = 0
    else:
        result_desc = "PUSH - Tie!"
        win_amount = bet_amount  # Return bet
    ...
    # Log game
    await log_game_session(user_id, "blackjack", bet_amount, win_amount, result_desc)
    await update_daily_house_stats(bet_amount, win_amount)
...
🃏 <b>Your Hand:</b> {hand_display(player_hand)} = <b>{player_value}</b>
🏠 <b>Dealer Hand:</b> {hand_display(dealer_hand)} = <b>{dealer_value}</b>

🎯 <b>Result:</b> {result_desc}
...
async def play_simple_roulette(user_id: int, bet_amount: float, bet_type: str) -> str:
    """Play a simple roulette game"""
    # Spin the wheel (0-36)
    ...
    if bet_type == 'red':
        is_win = winning_number in red_numbers
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "RED"
    elif bet_type == 'black':
        is_win = winning_number in black_numbers
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "BLACK"
    elif bet_type == 'even':
        is_win = winning_number != 0 and winning_number % 2 == 0
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "EVEN"
    elif bet_type == 'odd':
        is_win = winning_number != 0 and winning_number % 2 == 1
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "ODD"
    elif bet_type == 'low':
        is_win = 1 <= winning_number <= 18
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "LOW (1-18)"
    elif bet_type == 'high':
        is_win = 19 <= winning_number <= 36
        win_amount = bet_amount * 2.0 if is_win else 0
        bet_desc = "HIGH (19-36)"
    ...
    # Log game
    result_text = f"{'WIN' if is_win else 'LOSE'} - {bet_desc}"
    await log_game_session(user_id, "roulette", bet_amount, win_amount, result_text)
    await update_daily_house_stats(bet_amount, win_amount)
...
🎯 <b>Winning Number:</b> <b>{winning_number}</b> {get_color(winning_number)}
...
"""
        ...
        result = await play_simple_slots(user_id, bet_amount)
        
        keyboard = [
            [
                InlineKeyboardButton("🎰 Play Again", callback_data="play_slots"),
                InlineKeyboardButton("🎮 Games Menu", callback_data="games_menu")
            ]
        ]
        
        await query.edit_message_text(result, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
...
Choose your bet amount and type:
"""
        
        keyboard = [
            [InlineKeyboardButton("$1 HIGH", callback_data="dice_1_high"), InlineKeyboardButton("$1 LOW", callback_data="dice_1_low")],
            [InlineKeyboardButton("$5 HIGH", callback_data="dice_5_high"), InlineKeyboardButton("$5 LOW", callback_data="dice_5_low")],
            [InlineKeyboardButton("$10 HIGH", callback_data="dice_10_high"), InlineKeyboardButton("$10 LOW", callback_data="dice_10_low")],
            [InlineKeyboardButton("$25 LUCKY 7", callback_data="dice_25_seven")],
            [InlineKeyboardButton("🔙 Back to Games", callback_data="games_menu")]
        ]
        ...
        result = await play_simple_dice(user_id, bet_amount, bet_type)
    ...
    elif query.data == "play_blackjack":
        text = f"""
🃏 <b>BLACKJACK</b> 🃏
...
"""
        ...
        result = await play_simple_blackjack(user_id, bet_amount)
        
        keyboard = [
            [
                InlineKeyboardButton("🃏 Play Again", callback_data="play_blackjack"),
                InlineKeyboardButton("🎮 Games Menu", callback_data="games_menu")
            ]
        ]
...
🎯 <b>Betting Options:</b>
• RED/BLACK: 2.0x payout
• EVEN/ODD: 2.0x payout
• LOW (1-18)/HIGH (19-36): 2.0x payout

Choose your bet:
"""
        
        keyboard = [
            [InlineKeyboardButton("$5 RED", callback_data="roulette_5_red"), InlineKeyboardButton("$5 BLACK", callback_data="roulette_5_black")],
            [InlineKeyboardButton("$5 EVEN", callback_data="roulette_5_even"), InlineKeyboardButton("$5 ODD", callback_data="roulette_5_odd")],
            [InlineKeyboardButton("$10 LOW", callback_data="roulette_10_low"), InlineKeyboardButton("$10 HIGH", callback_data="roulette_10_high")],
            [InlineKeyboardButton("$25 RED", callback_data="roulette_25_red"), InlineKeyboardButton("$25 BLACK", callback_data="roulette_25_black")],
            [InlineKeyboardButton("🔙 Back to Games", callback_data="games_menu")]
        ]
        ...
        result = await play_simple_roulette(user_id, bet_amount, bet_type)
        
        keyboard = [
            [
                InlineKeyboardButton("🎡 Play Again", callback_data="play_roulette"),
                InlineKeyboardButton("🎮 Games Menu", callback_data="games_menu")
            ]
        ]
...
async def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("games", lambda u, c: start_game_menu(u, c)))
        ...
        # Add callback query handlers (order matters!)
        ...
        application.add_handler(CallbackQueryHandler(handle_game_callbacks, pattern="^(games_menu|play_|slots_bet_|dice_|blackjack_|roulette_)"))
        application.add_handler(CallbackQueryHandler(handle_main_callbacks))
        
        # Add text message handler (lowest priority)
        ...
        logger.info(f"🎮 Games Available: Slots, Dice (Blackjack & Roulette coming soon)")
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            for column in required_columns:
                if column not in existing_columns:
                    if 'today' in column or column == 'last_daily_reset':
                        default_value = "DEFAULT CURRENT_TIMESTAMP" if column == 'last_daily_reset' else "DEFAULT 0"
                    else:
                        default_value = "DEFAULT 0.0"
                    
                    alter_sql = f"ALTER TABLE house_balance ADD COLUMN {column} REAL {default_value}"
                    if column == 'last_daily_reset':
                        alter_sql = f"ALTER TABLE house_balance ADD COLUMN {column} TIMESTAMP {default_value}"
                    elif 'games_played_today' in column:
                        alter_sql = f"ALTER TABLE house_balance ADD COLUMN {column} INTEGER DEFAULT 0"
                    
                    try:
                        await db.execute(alter_sql)
                        logger.info(f"Added column {column} to house_balance table")
                    except Exception as e:
