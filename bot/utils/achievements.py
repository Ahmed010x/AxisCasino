...
</b>Progress:</b> {earned_count}/{total_achievements} completed
</b>Total Rewards Earned:</b> {total_rewards} chips
...
async def show_achievement_notification(update: Update, achievement_ids: List[str]):
 ...
 notification_text = "</b>NEW ACHIEVEMENT(S) UNLOCKED!</b>\n\n"
 
 total_reward = 0
 for achievement_id in achievement_ids:
 achievement = ACHIEVEMENTS[achievement_id]
 notification_text += f"</b>{achievement['name']}</b>\n"
 notification_text += f" {achievement['description']}\n"
 notification_text += f" Reward: </b>{achievement['reward']} chips</b>\n\n"
 total_reward += achievement['reward']
 
 notification_text += f"</b>Total Bonus:</b> {total_reward} chips added to your balance!"
